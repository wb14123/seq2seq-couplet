...
class Model:
    ...
    def _init_train(self):
        ...
        with self.train_graph.as_default():
            ...
            self.train_op = tf.train.AdamOptimizer(learning_rate=self.learning_rate).apply_gradients(zip(clipped_gradients, params))
    ...
    def eval(self, train_step):
        with self.eval_graph.as_default():
            ...
            for step in range(0, self.eval_reader.data_size):
                ...
                target_seq = data['target_seq']
                target_seq_len = data['target_seq_len']
                outputs = self.eval_session.run(self.eval_output,
                                                feed_dict={self.eval_in_seq: in_seq,
                                                           self.eval_in_seq_len: in_seq_len})
                for i in range(len(outputs)):
                    output = outputs[i]
                    target = target_seq[i]
                    output_text = reader.decode_text(output,self.eval_reader.vocabs).split(' ')
                    target_text = reader.decode_text(target[1:], self.eval_reader.vocabs).split(' ')
            ...
            return bleu.compute_bleu(target_results, output_results) * 100
    ...
    def infer(self, text):
        ...
        with self.infer_graph.as_default():
            ...
            outputs = self.infer_session.run(self.infer_output,
                                             feed_dict={self.infer_in_seq: [in_seq],
